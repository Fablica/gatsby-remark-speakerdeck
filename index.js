...
const isSpeakerDeckLink = node => {
  return node.children.length === 1 && node.children[0].type === 'link' && node.children[0].url.match(speakerdeckRegexp) && node.children[0].children.length === 1 && node.children[0].children[0].type === 'text' && node.children[0].children[0].value === node.children[0].url;
};
